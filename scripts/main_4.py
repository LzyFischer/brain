# -*- coding: utf-8 -*-
# @Time    : 2/15/2024 6:54 PM
# @Author  : Gang Qu
# @FileName: main_4.py
# MGCN regression interpretability experiment using two IQ scores
import argparse
import utils
import os
...
def main(args):
    ...
    mask_filename = r'F:\projects\AiyingT1MultimodalFusion\scripts\results\hcdp\cam\symmetric_mask.npy'
    ...
    for iter, sample_i in enumerate(train_loader):
        ...
        activations, gradients = grad_cam_util.generate_grad_cam(gs, gs.ndata['x'], target_layer_index=1, task_type='regression')

        all_activations.append(activations.cpu().detach().numpy())
        all_gradients.append(gradients.cpu().detach().numpy())
    all_activations = np.array(all_activations)
    all_gradients = np.array(all_gradients)
    print(all_activations.shape)
    np.save(r'F:\projects\AiyingT1MultimodalFusion\scripts\results\hcdp\cam\activations.npy', all_activations)
    np.save(r'F:\projects\AiyingT1MultimodalFusion\scripts\results\hcdp\cam\gradients.npy', all_gradients)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--y_attribute', default=['nih_crycogcomp_ageadjusted', 'nih_fluidcogcomp_ageadjusted'],
    ...
    parser.add_argument('--alpha', default=0, help='L1 sparsity for mask')
    parser.add_argument('--beta', default=0, help='L2 sparsity for mask')
