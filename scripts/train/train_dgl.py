...
def train_epoch(
    ...
    for iter, sample_i in enumerate(tqdm(data_loader, desc="Training iterations!")):
        ...
        if task_type == "regression":
            ...
            raise ValueError(
                "Invalid task type. Choose from 'regression' or 'classification'."
            )
        mask_weights = (
            model.layers[0].raw_edge_weight
            if isinstance(model.layers[0], SymmetricMaskedGraphConv)
            else None
        )
...
if __name__ == "__main__":
    ...
    dataset3 = HCDP_DGL(
        ...
        buckets_dict=buckets,
        attributes_to_group=attributes_group,
    )
    ...
    train_dataloader = DataLoader(
        dataset3, batch_size=64, shuffle=True, collate_fn=collate_dgl
    )
    optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
    for e in range(10):
        epoch_loss, optimizer, gs = train_epoch(
            model,
            optimizer,
