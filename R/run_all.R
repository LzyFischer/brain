...
# ==== 参数 ====
npz_path <- "/scratch/vjd5zr/project/BrainProject/brain/results/attention_weights/disease_ADHD.npz"
# SC_FC_att_selectedlabel
target_n <- 378         # 只取前 360×360
pct_keep <- 0.0003      # 0.05% 的边
mode_sel <- "positive"  # 或 "abs"（按绝对值筛）
...
# ==== 参数 ====
npz_path <- "/scratch/vjd5zr/project/BrainProject/brain/results/attention_weights/SC_FC_att_selectedlabel.npz"
# SC_FC_att_selectedlabel
target_n <- 378         # 只取前 360×360
pct_keep <- 0.0003      # 0.05% 的边
mode_sel <- "positive"  # 或 "abs"（按绝对值筛）
